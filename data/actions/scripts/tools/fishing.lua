...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if targetId == 23785 then
		if player:getStorageValue(Storage.Quest.Dawnport.TheDormKey) == 2 then
			if math.random(100) >= 97 and (not useWorms or player:removeItem("worm", 1)) then
		...
		elseif math.random(100) <= math.min(math.max(10 + (player:getEffectiveSkillLevel(SKILL_FISHING) - 10) * 0.597, 10), 50)
			and (not useWorms or player:removeItem("worm", 1)) then
